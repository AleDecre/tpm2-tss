    ...
    const TPM2B_PUBLIC *inPublic)
{
    ...
    rval = CommonPreparePrologue(ctx, TPM2_CC_VIRT_LoadSeed);
    if (rval)
        return rval;
    ...
    TSS2L_SYS_AUTH_RESPONSE *rspAuthsArray)
{
    ...
    rval = Tss2_Sys_VIRT_LoadSeed_Prepare(sysContext, parentHandle, inPrivate, inPublic);
    if (rval)
        return rval;
    ...
    return Tss2_Sys_VIRT_LoadSeed_Complete(sysContext, objectHandle, name);
}
